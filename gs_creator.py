...
KEV_TO_MEV = 1000.0  # Conversion factor from keV to MeV, divide by this,
...
def create_spectrum_from_peaks(
    ...
    Parameters
    peak_energies  : List of gamma peak energies in keV
    emission_rates : List of emission rates (intensities) for each peak
    num_bins :  Number of bins in the spectrum
    energy_range : Energy range as (min_energy, max_energy) in keV.
                   If None, defaults to (0, max(peak_energies) * 1.5)
    background_spectrum : Optional background ph_spectrum object to add to the created spectrum
    fwhm_factor : Factor to calculate FWHM from energy (default 0.02 means 2% of energy)
    spec_name   : Name for the created spectrum (default "synthetic_spectrum")
    efficiency_coefficients : Efficiency fit coefficients to adjust peak heights based on energy.
                              If provided, peak heights will be scaled by the detector efficiency.
    efficiency_fit_type : Type of efficiency fit to use (1 or 2, default 1).
                          1: logarithmic fit, 2: inverse energy fit.
    include_compton : If True, generate Compton continuum for each peak (default False)
    compton_fraction : Fraction of peak intensity that goes into Compton continuum (default 0.5)

    Returns
    PhSpectrum:  A new PhSpectrum object with the synthetic gamma peaks
    """
    # Validate inputs
